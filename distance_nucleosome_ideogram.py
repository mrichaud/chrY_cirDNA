...
def plot_ideogram_nucleosome_count(df_long, suffix, centromere_file=None):
    ...
    for ax, chrom in zip(axes, chroms):
        ...
        # 6. SUPERPOSITION DES RÃ‰GIONS CENTROMÃ‰RIQUES EN GRIS
        if chrom in centro:
            ...
            for (c_start, c_end) in centro[chrom]:
                # Convertir coordonnÃ©es gÃ©nomiques â†’ coordonnÃ©es pixel (bins)
    ...
    cbar.set_label("Nombre de nuclÃ©osomes", fontsize=22, fontweight='bold', labelpad=8)
    cbar.ax.xaxis.set_label_position("top")
    cbar.ax.xaxis.set_ticks_position("top")

    # 9. LÃ‰GENDE CENTROMÃˆRE
    if centromere_file is not None:
        from matplotlib.patches import Patch
        legend_elements = [Patch(facecolor='grey', alpha=0.85, label='CentromÃ¨re')]
        fig.legend(
    ...
    # 10. SAUVEGARDER
    plt.savefig(
        f"ideogram_nucleosome_count_{suffix}.svg",
        dpi=300, bbox_inches="tight", facecolor="white"
    )
    plt.close()

    print(f"Saved ideogram_nucleosome_count_{suffix}.svg")
...
def plot_ideogram_nucleosome_distance(df_long, suffix):
    ...
    fig.subplots_adjust(left=0.08, top=0.97)  # â† rapproche titre et label Y
    ...
    cbar.ax.tick_params(labelsize=18)
    cbar.set_label("Distance moyenne entre nuclÃ©osomes (log2)", fontsize=22, fontweight='bold')

    # 6. LÃ‰GENDE POUR LES NaN
    legend_elements = [
        Patch(facecolor="#2166AC", edgecolor="black", linewidth=0.5,
              label="Distance courte (<128 bp) â€” NuclÃ©osomes serrÃ©s"),
        Patch(facecolor="#F7F7F7", edgecolor="black", linewidth=0.5,
              label="Distance normale (~200 bp) â€” Espacement canonique"),
        Patch(facecolor="#B2182B", edgecolor="black", linewidth=0.5,
              label="Distance longue (>1 kb) â€” HÃ©tÃ©rochromatine"),
        Patch(facecolor=nan_color, edgecolor="black", linewidth=1,
              label="DonnÃ©es manquantes (NaN)")
    ]
    ...
    # 7. TITRE
    fig.suptitle(
        "Distribution spatiale de la distance moyenne des nuclÃ©osomes (log2) le long des chromosomes (bins de 10kb)",
        fontsize=16, fontweight="bold", y=1.00)

    # 8. SAUVEGARDER
    plt.savefig(
        f"ideogram_average_distance_{suffix}.svg",
        dpi=300, bbox_inches="tight", facecolor="white"
    )
    plt.close()

    print(f"Saved ideogram_average_distance_{suffix}.svg")
...
if __name__ == "__main__":
    ...
    tsv_dir = Path("/data2/USERS/lruffinel/cfDNA/chromosomes_cover_uncover/distance_nucleosome_norm/")
    tsv_dir.mkdir(exist_ok=True)

    # 1) Charger tous les TSV
    df_long = load_nucleosome_count_table(tsv_dir)
    df_long.to_csv("df_long_nucleosome_count_global.csv", index=False)

    df_long_1 = load_average_distance_table(tsv_dir)
    df_long_1.to_csv("df_long_average_distance_global.csv", index=False)

    # 2) Tracer les plots style idÃ©ogramme (haute rÃ©solution)

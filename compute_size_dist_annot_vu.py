...
# ============================
ALL_FEATURES = [
    "GapArtf", "Quies", "HET", "ReprPC", "Acet", "EnhW", "EnhA", 
    "TxEnh", "TxWk", "TxEx", "Tx", "znf", "DNase", "BivProm", "PromF", "TSS"
]
...
def load_interval_tree(annot_filepath: str):
    ...
    df["chr"] = df["chr"].astype(str).apply(lambda c: c if c.startswith("chr") else f"chr{c}")
    ...
    df["feature"] = df["annotation"].apply(assign_feature)
...
CHROM_TABLES = None
ANNOT_PATH = "hg38_genome_100_browser2.bed"
OUT_DIR = None
...
def process_single_patient(indiv):
    ...
    for chrom in all_chroms:
        ...
        for f_idx, feat in enumerate(ALL_FEATURES):
            ...
            np.savetxt(f"{OUT_DIR}/{sname}_{chrom}_{feat}_size.tsv", norm_feat, fmt="%.6f")
            ...
            np.savetxt(f"{OUT_DIR}/{sname}_{chrom}_{feat}_Other_size.tsv", norm_other, fmt="%.6f")
...
def plot_chrom_multi_features(
  ...
  for feat in target_feats:
    files = sorted(glob.glob(f"{cond_out}/*_{chrom_label}_{feat}_size.tsv"))
    if not files:
      continue
    ...
    if matrix.size > 0:
      ...
      out_tsv = os.path.join(
          output_tsv_dir, f"{cond_key}_{chrom_label}_{feat}_mean_size.tsv"
      )
  ...
  if other_matrices:
    ...
    other_tsv = os.path.join(
        output_tsv_dir, f"{cond_key}_{chrom_label}_Other_mean_size.tsv"
    )
...
if __name__ == "__main__":
    ...
    all_chroms = [f"chr{i}" for i in ["19","17","22"]]
    ...
    for cond_key, cfg in conditions.items():
        ...
        target_features = ["HET", "PromF", "TSS", "DNase"]
        ...
        for chrom in all_chroms:
          plot_chrom_multi_features(
              ...
              output_plot_name=os.path.join(
                  plots_chrom_dir,
                  f"{cond_key}_{chrom}_HET_PromF_TSS_Txex_DNase_vs_Other.svg",
              ),

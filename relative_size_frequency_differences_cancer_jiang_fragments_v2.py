...
def process_curves_and_hists(name, files_or_pattern, L=1000, n_rep=1000, seed=0, score_min=30, remove_par=True):
    ...
    print(f"\n=== Cohorte Jiang {name} : {len(all_files)} fichiers ===")
...
def plot_superposed_diff(data, color_map, label_map, alpha_band=0.20, xlim=(50, 250), out_svg=None):
    ...
    plt.title(
        "Superposition courbes de fr̩quences relatives des donn̩es Jiang (male) : freq(Autosomes+X) - freq(Y)\n"
    ...
    plt.legend(handles, labels, frameon=False, title="Disease", title_fontsize=10, fontsize=9, ncol=2)
    plt.tight_layout()
...
if __name__ == "__main__":
    ...
    liver_cancer = select_files(annot, cancer_label="Liver", pattern="/data2/USERS/richaud/finaledb/liver/jiang/*.hg38.frag.tsv")
    ...
    plot_superposed_diff(data, color_map=color_map, label_map=label_map, alpha_band=ALPHA_BAND, xlim=(50, 250), out_svg="relative_size_frequency_diff_superposed_jiang.svg")

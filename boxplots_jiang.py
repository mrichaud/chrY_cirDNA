...
if __name__ == "__main__":
    OUT_ROOT = "/data2/USERS/lruffinel/chrY/distribution_fragments/jiang/"

    conditions = {
        "Healthy": {"pretty": "Healthy"},
        "Cirrhosis": {"pretty": "Cirrhosis"},
        "Hepatitis B": {"pretty": "Hepatitis B"},
        "Liver": {"pretty": "Liver"},
    }
    ...
    for cond_key, cfg in conditions.items():
        ...
        cond_dir = os.path.join(OUT_ROOT, cond_key)

        print(f"\n[{cond_pretty}] reading profiles in: {cond_dir}")
        print("  n chrX:", len(glob.glob(os.path.join(cond_dir, "*_chrX_size.tsv"))))
        print("  n autoX:", len(glob.glob(os.path.join(cond_dir, "*_chr_Auto_X_size.tsv"))))
        print("  n chrY:", len(glob.glob(os.path.join(cond_dir, "*_chrY_size.tsv"))))

        df_paths = build_table(cond_dir)
        ...
        # --- Boxplots (Y vs X)
        make_boxplot(
            df["Y_100_147"], df["X_100_147"],
            "Y", "X",
            "Proportion of fragments",
            f"{cond_pretty} — 100–147 bp (n={n})",
            os.path.join(cond_dir, f"{cond_key}_boxplot_100_147_Y_vs_X.svg"),
            pvalue=p_Y_vs_X_100_147
        )

        make_boxplot(
            df["Y_147_200"], df["X_147_200"],
            "Y", "X",
            "Proportion of fragments",
            f"{cond_pretty} — 147–200 bp (n={n})",
            os.path.join(cond_dir, f"{cond_key}_boxplot_147_200_Y_vs_X.svg"),
            pvalue=p_Y_vs_X_147_200
        )

        make_boxplot(
            df["Y_ratio"], df["X_ratio"],
            "Y", "X",
            "Ratio",
            f"{cond_pretty} — Ratio (100–147 / 147–200) (n={n})",
            os.path.join(cond_dir, f"{cond_key}_boxplot_ratio_Y_vs_X.svg"),
            pvalue=p_Y_vs_X_ratio
        )

        # --- Boxplots (Y vs Auto+X)
        make_boxplot(
            df["Y_100_147"], df["AutoX_100_147"],
            "Y", "Auto+X",
            "Proportion of fragments",
            f"{cond_pretty} — 100–147 bp (n={n})",
            os.path.join(cond_dir, f"{cond_key}_boxplot_100_147_Y_vs_AutoX.svg"),
            pvalue=p_Y_vs_AutoX_100_147
        )

        make_boxplot(
            df["Y_147_200"], df["AutoX_147_200"],
            "Y", "Auto+X",
            "Proportion of fragments",
            f"{cond_pretty} — 147–200 bp (n={n})",
            os.path.join(cond_dir, f"{cond_key}_boxplot_147_200_Y_vs_AutoX.svg"),
            pvalue=p_Y_vs_AutoX_147_200
        )

        make_boxplot(
            df["Y_ratio"], df["AutoX_ratio"],
            "Y", "Auto+X",
            "Ratio",
            f"{cond_pretty} — Ratio (100–147 / 147–200) (n={n})",
            os.path.join(cond_dir, f"{cond_key}_boxplot_ratio_Y_vs_AutoX.svg"),
            pvalue=p_Y_vs_AutoX_ratio
        )

    # --- Sauvegarde tableau p-values
    ...
    print("\nTerminé: boxplots + p-values Wilcoxon générés à partir des profils existants.")

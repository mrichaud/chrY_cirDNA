...
ROOT = "/data2/USERS/lruffinel/cfDNA/distribution_fragments/cristiano/fragments_tsv"
GROUP_DIRS = {
    "healthy": os.path.join(ROOT, "healthy"),
    "colorectal": os.path.join(ROOT, "colorectal_cancer"),
    "lung": os.path.join(ROOT, "lung_cancer"),
    "pancreatic": os.path.join(ROOT, "pancreatic_cancer"),
}

OUT_DIR = os.path.join("/data2/USERS/richaud/chrY/distribution_fragments/comparison_ratio_2")
os.makedirs(OUT_DIR, exist_ok=True)

SHORT_WIN = (100, 147)     # 0..146
LONG_WIN  = (147, 200)   # 147..299
EPS = 1e-12
...
def plot_scatter_2groups(df_h, df_c, cancer_name, out_svg, use_log=False):
    ...
    ax.set_title(f"Short vs Long fragments Y/AutoX ({suffix})\n"
                 f"healthy vs {cancer_name} (cristiano)", fontweight="bold", size=11)
    
    ax.set_xlabel(f"Long fragments {suffix} (Y/AutoX) on {LONG_WIN[0]}â€“{LONG_WIN[1]} bp")
    ax.set_ylabel(f"Short fragments {suffix} (Y/AutoX) on {SHORT_WIN[0]}â€“{SHORT_WIN[1]} bp")
...
def plot_scatter_4groups(df_all, out_svg, use_log=False):
    x_col = "logratio_long" if use_log else "ratio_long"
    y_col = "logratio_short" if use_log else "ratio_short"
    suffix = "logratio" if use_log else "ratio"

    palette = {"healthy":"#1f77b4","colorectal":"#d62728","lung":"#ff7f0e","pancreatic":"#2ca02c"}

    fig, ax = plt.subplots(figsize=(6.2, 6.2))
    for g, sub in df_all.groupby("group"):
        ax.scatter(sub[x_col], sub[y_col], s=35, alpha=0.85, color=palette.get(g,"grey"),
                   label=f"{g} (n={len(sub)})", edgecolors="none")

    ax.set_title("Short and Long fragments ratio(Y/AutoX) â€” 4 groups (cristiano)", fontweight="bold", size=11)
    ax.set_xlabel(f"Long fragments {suffix} (Y/AutoX) on {LONG_WIN[0]}â€“{LONG_WIN[1]} bp")
    ax.set_ylabel(f"Short fragments {suffix} (Y/AutoX) on {SHORT_WIN[0]}â€“{SHORT_WIN[1]} bp")
    ax.grid(True, alpha=0.25)
    ax.legend(frameon=False)
...
if __name__ == "__main__":
    ...
    # CSV global
    out_csv = os.path.join(OUT_DIR, "scalar_ratios_short_long_ratio_147bp_cristiano.csv")
    ...
    for cancer in ["colorectal", "lung", "pancreatic"]:
        ...
        plot_kde_two_groups(
            ...
            title=f"short fragments ({SHORT_WIN[0]}â€“{SHORT_WIN[1]} bp) : healthy vs {cancer} (cristiano)",
            out_svg=os.path.join(OUT_DIR, f"ratio_{SHORT_WIN[0]}_{SHORT_WIN[1]}_healthy_vs_{cancer}_cristiano.svg"),
            x_ref=None,
        )
        ...
        plot_kde_two_groups(
            ...
            title=f"long fragment ({LONG_WIN[0]}â€“{LONG_WIN[1]} bp) : healthy vs {cancer} (cristiano)",
            out_svg=os.path.join(OUT_DIR, f"ratio_{LONG_WIN[0]}_{LONG_WIN[1]}_healthy_vs_{cancer}_cristiano.svg"),
            x_ref=None,
        )
        ...
        plot_scatter_2groups(
            df_h, df_c, cancer,
            os.path.join(OUT_DIR, f"scatter_ratio_short_vs_long_healthy_vs_{cancer}_147bp_cristiano.svg"),
            use_log=False
        )

    # scatters 4 groupes (ratio + logratio)
    plot_scatter_4groups(
        df_all,
        os.path.join(OUT_DIR, "scatter_ratio_short_vs_long_4groups_147bp_cristiano.svg"),
        use_log=False
    )
